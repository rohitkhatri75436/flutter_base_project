...
//getDate in '15-Jan-2021' date format
  ...
  static String? comparedChatDateTime(String timestamp) {
    final tm = DateTime.fromMillisecondsSinceEpoch(int.parse(timestamp)).toLocal();
    final DateTime today = DateTime.now();
    const Duration oneDay = Duration(days: 1);
    const Duration twoDay = Duration(days: 2);
    const Duration oneWeek = Duration(days: 7);
...
//getDate in '12-01-2021' format
  static String getSimpleDate(String tm) {
    final DateTime dateTime = DateTime.parse('${tm}Z');
    final formattedDate = DateFormat('dd MMM yyyy').format(dateTime);
    return formattedDate;
  }

  static String getSimpleDateWithTime(String tm) {
    final DateTime dateTime = DateTime.parse('${tm}Z');
    final formattedDate = DateFormat('dd MMM yyyy hh:mm aa').format(dateTime);
    return formattedDate;
  }
...
//getDateTime in 'Mon, 14:20' format
  ...
  static String convertToAgo(String? input) {
    if (input != null) {
      final DateTime? inputDate = DateTime.tryParse('${input}Z');

      if (inputDate != null) {
        final Duration diff = DateTime.now().toUtc().difference(inputDate);
  ...
  static String getLastMessageTime(int timestamp) {
  ...
  static String getTimeString(int timeInMillis) {
    final DateFormat formatter = DateFormat('hh:mmaa');
    DateTime date = DateTime.fromMillisecondsSinceEpoch(timeInMillis);
    return formatter.format(date);
  }

  static String getDateString(int timeInMillis) {
    final DateFormat formatter = DateFormat('dd MMM yyyy');
    DateTime date = DateTime.fromMillisecondsSinceEpoch(timeInMillis);
    return formatter.format(date);
  }

  static String getDateTimeString(int timeInMillis) {
  ...
  static String formatTimestamp(int timestamp) {
    ...
    final formattedDate = DateFormat("dd MMM yyyy, hh:mm aa").format(dateTime);
    return formattedDate;
    // Date Formate : 14 Mar 2022, 12:30
  }
}
